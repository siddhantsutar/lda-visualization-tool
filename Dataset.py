...
class Dataset(object):
    def __init__(self, filename):
        ...
        self.model = None
        self.dataCleanup()
        self.uniqueTypes = (self.data["Importance/Type"].unique())[:-2]
        self.uniqueYears = []

    def dataCleanup(self):
        self.data[self.data["Issue_ID"].apply(lambda x: str(x).isdigit())]
        self.data = self.data[self.data != "null"]
        self.data["Title/Description"] = self.data["Title/Description"].str.replace("quot", "")
        self.data["Creation_Date"] = self.data["Creation_Date"].apply(lambda x: int((str(x).split(' ')[0].split('/')[2]) if isinstance(x, basestring) else 0))

    def updateByType(self, getType):
        self.typeData = self.data[self.data["Importance/Type"] == getType]
        self.uniqueYears = sorted(self.typeData["Creation_Date"].unique())[1:]
        
    def fitLDA(self, nTopics, nTopWords):
        topicsList = []
    ...
    def getDataPoints(self, topic, typeID):
        dataPoints = None
        if typeID == 1: #By time
